...
def get_db_connection():
    # MySQL 연결 설정
    DATABASE_URL = "mysql+pymysql://scott:tiger@localhost:3310/cache_db"  # 실제 연결 정보로 변경
...
def find_similar_doc_from_db(query: str):
    ...
    return "캐시에 유사한 질문이 있습니다:: \n질문 :: " + results[0]['query'] + "\n결과 ::" + results[0]['created_answer']
...
def get_message_from_llm(query: str):
    ...
    # LLM 모델 호출
    answer = chain.invoke(query).replace('  ', ' ').replace("'", "`").replace('"', '`')
